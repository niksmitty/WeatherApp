...
static NSString * const OWMAPIBaseURLString = @"http://api.openweathermap.org/data/";
...
-(instancetype)initWithApiKey:(NSString*)apiKey {
    ...
    if (self) {
        self = [[OpenWeatherMapAPIManager alloc] initWithBaseURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@/", OWMAPIBaseURLString, OWMAPIVersion]]];
...
-(void)getCurrentWeatherByCityname:(NSString*)name withCompletionHandler:(void (^)(NSError *error, NSDictionary *result))completion {
    NSString *urlPart = [NSString stringWithFormat:@"weather?q=%@", name];
